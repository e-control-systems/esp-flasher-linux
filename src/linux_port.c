...
static void transfer_debug_print(const uint8_t *data, uint16_t size,
                                 bool write) {
...
static struct gpiod_line *reset_trigger_line;
static struct gpiod_chip *reset_trigger_chip;
static struct gpiod_chip *boosel_trigger_chip;
static struct gpiod_line *bootsel_trigger_line;
...
esp_loader_error_t linux_loader_port_init(const loader_linux_config_t *config) {
  ...
  reset_trigger_chip =  gpiod_chip_open_by_name(config->reset_trigger_chip);
  if (reset_trigger_chip == NULL){
    perror("Reset trigger chip could not be opened!\n");
    return ESP_LOADER_ERROR_FAIL;
  }
  reset_trigger_line = gpiod_chip_get_line(reset_trigger_chip, config->reset_trigger_line);
  gpiod_line_request_output(reset_trigger_line, "esp-reset-pin", SERIAL_FLASHER_RESET_INVERT ? 1 : 0);
  boosel_trigger_chip = gpiod_chip_open_by_name(config->bootsel_trigger_chip);
  bootsel_trigger_line = gpiod_chip_get_line(boosel_trigger_chip, config->bootsel_trigger_line);
  gpiod_line_request_output(reset_trigger_line, "esp-bootsel-pin", SERIAL_FLASHER_BOOT_INVERT ? 0 : 1);
...
void linux_loader_port_deinit(void) {
  ...
  gpiod_chip_close(boosel_trigger_chip);
}
...
void loader_port_enter_bootloader(void) {
  gpiod_line_set_value(bootsel_trigger_line, SERIAL_FLASHER_BOOT_INVERT ? 1 : 0);
  loader_port_reset_target();
  loader_port_delay_ms(SERIAL_FLASHER_BOOT_HOLD_TIME_MS);
  gpiod_line_set_value(bootsel_trigger_line, SERIAL_FLASHER_BOOT_INVERT ? 0 : 1);
}

void loader_port_reset_target(void) {
  gpiod_line_set_value(reset_trigger_line, SERIAL_FLASHER_RESET_INVERT ? 1 : 0);
  loader_port_delay_ms(SERIAL_FLASHER_RESET_HOLD_TIME_MS);
  gpiod_line_set_value(reset_trigger_line, SERIAL_FLASHER_RESET_INVERT ? 0 : 1);
}
