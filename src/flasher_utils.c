...
static const uint32_t bootloader_addresses[] = {
	[ESP8266_CHIP] = 0x0,	 [ESP32_CHIP] = 0x1000, [ESP32S2_CHIP] = 0x1000,
	[ESP32C3_CHIP] = 0x0,	 [ESP32S3_CHIP] = 0x0,	[ESP32C2_CHIP] = 0x0,
	[ESP32C5_CHIP] = 0x2000, [ESP32H2_CHIP] = 0x0,	[ESP32C6_CHIP] = 0x0,
	[ESP32P4_CHIP] = 0x2000
};

// If someone adds a new chip but forgets to update the array, compilation FAILS
_Static_assert(
	sizeof(bootloader_addresses) / sizeof(bootloader_addresses[0]) ==
		ESP_MAX_CHIP,
...
esp_loader_error_t connect_to_target(uint32_t higher_transmission_rate)
{
	...
	(defined SERIAL_FLASHER_INTERFACE_USB)
	if (higher_transmission_rate &&
	    esp_loader_get_target() != ESP8266_CHIP) {
	...
	(defined SERIAL_FLASHER_INTERFACE_USB)
esp_loader_error_t
connect_to_target_with_stub(const uint32_t current_transmission_rate,
			    const uint32_t higher_transmission_rate)
{
	...
	if (higher_transmission_rate != current_transmission_rate) {
		err = esp_loader_change_transmission_rate_stub(
			current_transmission_rate, higher_transmission_rate);
...
esp_loader_error_t load_ram_binary(const uint8_t *bin)
{
	...
	esp_loader_error_t err;
	const esp_loader_bin_header_t *header =
		(const esp_loader_bin_header_t *)bin;
	...
	uint32_t offset = esp_loader_get_target() == ESP8266_CHIP ?
				  BIN_HEADER_SIZE :
				  BIN_HEADER_EXT_SIZE;
	for (seg = 0, cur_seg_pos = (uint32_t *)(&bin[offset]);
	     seg < header->segments; seg++) {
	...
	for (seg = 0; seg < header->segments; seg++) {
		...
		err = esp_loader_mem_start(segments[seg].addr,
					   segments[seg].size, ESP_RAM_BLOCK);
		if (err != ESP_LOADER_SUCCESS) {
